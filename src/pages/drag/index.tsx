...
const sourceList = new Array(100).fill(new Array(1).fill(`速度速度速度速度${Math.random() +Math.random() }`))
console.log(sourceList);

const Drag = () => {
  const [data,setData] = useState<any>(sourceList.slice(0,10))
  const [transForm,setTransForm] = useState<string>('')
  const [totalHeight,setTotalHeight] = useState<number>(sourceList.length * itemHeight)
  ...
  return <div className={styles.dragWrap}>
      <div className={styles.dragBox} onScroll={showScroll} ref={scrollWrap}>
        <div style={{height:totalHeight}} className={styles.positionWrap}></div>
        <div  style={{transform:transForm}} className={styles.content}>
        {
            data.map((item:any,index:number) => {
              return <div key={index}>{item.value} +{index}</div>
            })
          }
        </div>
